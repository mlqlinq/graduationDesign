...
import { getAllPoorData } from "@/http/api/nationalGrants/poorStudCertific";
...
const getAllData = async () => {
	...
	await getAllPoorData({ schoolName: universities, userIdentity: userData.value.userIdentity, guideCollege: guide_college })
		.then((result) => {
...
const handleEdit = (index, row) => {
	router.push({ path: "/confirmationFilling", query: { Num: parseInt(userData.value.userIdentity), id: row.id } });
...
onBeforeRouteLeave((to, from) => {
	if (to.path !== "/confirmationFilling") return;
	to.meta.title = "查看页";
});
...
const handleDelete = (index, row) => {
	if (userData.value.userIdentity === "1") {
		if ("school_review_comments" in row && row.school_review_comments !== "") {
		...
		if ("reasons_for_democratic_review" in row && row.reasons_for_democratic_review !== "") {
			return ElNotification({
				title: "温馨提示",
				message: "您已经审核过啦",
				type: "success"
...
const submitForReview = () => {
	...
	if (userData.value.userIdentity === "1") {
		postPoorStudCertificSchoolExamine(ruleForm);
	} else if (userData.value.userIdentity === "2") {
		postPoorStudCertificDepartmentExamine(ruleForm);
	} else if (userData.value.userIdentity === "3") {
		postPoorStudCertificClassExamine(ruleForm);
	}
	delete ruleForm.id;
