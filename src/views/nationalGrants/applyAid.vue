<template>
	<div class="university">
		...
		<el-card class="tableCard">
			<el-table ref="taskTableRef" :data="applyAidData" :header-cell-style="{ background: '#e4e8e9', borderColor: '#cfcfcf' }" border :height="tableH" empty-text="没有数据" style="width: 100%" @select="selectClick">
				...
				<el-table-column prop="student_major" label="专业名称" align="center " />
...
import { getMyapplyData } from "@/http/api/nationalGrants/applyAid";
	...
	await getMyapplyData(query)
		.then((res) => {
			...
			if (res.data) {
				...
				if (applyAidData.value.length === 0) {
					tableH.value = 200;
				}
			}
		})
...
// 填写申请
const upLoadMy = () => {
	router.push("/financialAidFilling");
	sessionStorage.setItem("activePath", "/financialAidFilling");
};
